...
function addTaskInputRow(containerElement) {
    ...
    const newInput = containerElement.querySelector(".taskInputRow:last-child .taskTextInputNew");
    // Temporarily commented out focus as it might interfere with blur/enter flow
    // if (newInput) {
        ...
        const handleTitleSave = () => {
            ...
            if (currentProjIdForModal === null || currentProjIdForModal === undefined) {
                ...
                if (createdProject && createdProject.id !== undefined) {
                    ...
                    if (tasks.length > 0) {
                        console.log(`Updating new project ${currentProjIdForModal} with tasks:`, tasks);
